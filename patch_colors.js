...
const replace = `                let previewHtml = \`<div class="color-preview" style="background: \${item.acc || item.color || item.value}; width: 40px; height: 10px; border-radius: 5px; margin: 0 auto 5px auto;"></div>\`;
                if(tab === 'bubbles') {
                    // Show a split gradient of bubble background and text color
                    const bg = item.color || 'var(--island)';
                    const text = item.text || '#fff';
                    previewHtml = \`<div class="color-preview" style="background: linear-gradient(135deg, \${bg} 50%, \${text} 50%); width: 40px; height: 10px; border-radius: 5px; margin: 0 auto 5px auto; border: 1px solid var(--glass-border);"></div>\`;
                } else if(tab === 'themes') {
                    // Show a split gradient of theme background and accent color
                    const bg = item.bg || 'var(--bg-dark)';
                    const acc = item.acc || 'var(--accent-yellow)';
                    previewHtml = \`<div class="color-preview" style="background: linear-gradient(135deg, \${bg} 50%, \${acc} 50%); width: 40px; height: 10px; border-radius: 5px; margin: 0 auto 5px auto; border: 1px solid var(--glass-border);"></div>\`;
                } else if(tab === 'badges') {`;
