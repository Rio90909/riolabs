...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        page.evaluate("""
            ...
            // Re-define adminPopulateEditDropdown
            const origLoad = window.adminPopulateEditDropdown;
            window.adminPopulateEditDropdown = function() {
                console.log("adminPopulateEditDropdown called!");
                origLoad();
            };
        ...
        page.evaluate("""
            ...
            // Re-define adminUpdateShopFields to see if it's called somewhere else
            const oUpdate = window.adminUpdateShopFields;
            window.adminUpdateShopFields = function(skip) {
                console.log("adminUpdateShopFields called with skip = " + skip);
                console.trace();
                oUpdate(skip);
            };
            ...
            setTimeout(() => {
                ...
                console.log("Done. select value: " + document.getElementById('adminShopEditSelect').value);
            }, 100);
        """)
